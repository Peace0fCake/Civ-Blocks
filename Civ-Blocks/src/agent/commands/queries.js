...
export const queryList = [
    ...
    {
        name: "!nearbyBlocks",
        description: "Get the block types near the bot.",
        perform: function (agent) {
            let bot = agent.bot;
    ...
    {
        name: "!nearbyBlocksPositions",
        description: "Get the blocks near the bot and their positions.",
        perform: function (agent) {
            let bot = agent.bot;
            let res = 'NEARBY_BLOCKS';
            let blocks = world.getNearbyBlockTypesV2(bot, 6);
            for (let i = 0; i < blocks.length; i++) {
                res += `\n- ${blocks[i]}`;
            }
            if (blocks.length == 0) {
