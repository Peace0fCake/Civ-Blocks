...
import { actionsList } from './actions2.js';
...
export function containsCommand(message) {
    if (!message || typeof message !== 'string') {
        console.warn("Invalid input passed to containsCommand:", message);
        return null;
    }
...
export function getCommandDocs2() {
    const typeTranslations = {
        //This was added to keep the prompt the same as before type checks were implemented.
        //If the language model is giving invalid inputs changing this might help.
        'float':        'number',
    ...
    Use the commands with the syntax: !commandName or !commandName("arg1", 1.2, ...) if the command takes arguments.\n
    Do not use codeblocks. Only use one command in each response, trailing commands and comments will be ignored.\n`;
    const blockedCommands = ['!goal', 'endGoal', '!restart'];
    for (let command of actionsList) {
        ...
        if (command.params) {
          parameters = '(';       
            let param_array = Object.keys(command.params);
            parameters += (param_array.length > 0) ? param_array.join(', ') : '';
            parameters += ')';
        }
        docs += command.name + parameters +': '+ command.description + '\n';
        if (command.params) {
            docs += 'Parameters\n';
            for (let param in command.params) {
                docs += `${param}: (${typeTranslations[command.params[param].type]??command.params[param].type}) ${command.params[param].description}\n`;
