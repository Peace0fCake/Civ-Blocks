...
export function getNearestBlocksV2(bot, block_types=null, distance=16, count=10000) {
    /**
     * Get a list of the nearest blocks of the given types.
     * @param {Bot} bot - The bot to get the nearest block for.
     * @param {string[]} block_types - The names of the blocks to search for.
     * @param {number} distance - The maximum distance to search, default 16.
     * @param {number} count - The maximum number of blocks to find, default 10000.
     * @returns {string} - The nearest blocks of the given type in JSON format.
     * @example
     * let woodBlocks = world.getNearestBlocksV2(bot, ['oak_log', 'birch_log'], 16, 1);
     **/
    // if blocktypes is not a list, make it a list
    let block_ids = [];
    if (block_types === null) {
    ...
    for (let i = 0; i < positions.length; i++) {
        let block = bot.blockAt(positions[i]);
        blocks.push({ block_name: block.name, position: positions[i] });
    }

    let res = [];
    for (let i = 0; i < blocks.length; i++) {
        res.push({ block_name: blocks[i].block_name, position: blocks[i].position });
    }
...
export function getNearbyBlockTypesV2(bot, distance=16) {
    /**
     * Get a list of all nearby block names.
     * @param {Bot} bot - The bot to get nearby blocks for.
     * @param {number} distance - The maximum distance to search, default 16.
     ...
     * let blocks = world.getNearbyBlockTypes(bot);
     **/
    let blocks = getNearestBlocksV2(bot, null, distance);
